...
*                                                                            *
\****************************************************************************/
 lex.c,v 1.190 2022/10/25 14:32:30 ssb Exp $ */
#include <stdlib.h>
#include <string.h>
#include <ctype.h>
#include <errno.h>
#include "GOOMBAServer.h"
#ifdef GOOMBAServer_HAVE_MMAP
#include <sys/mman.h>
#endif
#ifdef HAVE_UNISTD_H
#include <unistd.h>
 ...
 * command.
 */
#define GOOMBAServer_MAX_CMD_LEN 21
#define GOOMBAServer_MAX_CMD_NUM 39

static cmd_table_t cmd_table[GOOMBAServer_MAX_CMD_LEN+1][GOOMBAServer_MAX_CMD_NUM+1] = {
	{ { NULL,0,NULL } },        /* 0 */
	...
	{ { "echo",GOOMBAServerECHO,NULL },     /* 4 */
	  ...
	  { "prev",PREV,NULL },
	  { "sort",GOOMBAServerSORT,NULL },
	  { "rand",INTFUNC0,Rand },
	  { "sqrt",INTFUNC1,Sqrt },
	  { "file",INTFUNC1,GOOMBAServerFile },
	  { "link",INTFUNC2,Link },
	  { "mail",GOOMBAServerMAIL,NULL },
	  { "chop",INTFUNC1,Chop },
	  { "ceil",INTFUNC1,Ceil },
	  { NULL,0,NULL } }, 
	...
	{ { "endif",ENDIF,NULL },   /* 5 */
	  ...
	  { "flush",INTFUNC0,GOOMBAServerFlush },
	  { "eregi",EREGI,NULL },
	  { "crypt",CRYPT,NULL },
	  ...
	  { "asort",GOOMBAServerASORT,NULL },
	  { "umask",UMASK,NULL },
	  { "logas", INTFUNC1,LogAs },
	  { "rsort",GOOMBAServerRSORT,NULL },
	...
	{ { "elseif",ELSEIF,NULL }, /* 6 */
	  ...
	  { "arsort",GOOMBAServerARSORT,NULL },
	  { NULL,0,NULL } }, 
	...
	{ { "default", DEFAULT,NULL }, /* 7 */
	  ...
	  { "snmpget", INTFUNC3,GOOMBAServersnmpget },
#endif
#if HAVE_FILEPRO
...
#if GOOMBAServer_SNMP_SUPPORT
	  { "snmpwalk", INTFUNC3,GOOMBAServersnmpwalk },
#endif
	...
	{ { "clearstack", INTFUNC0,ClearStack }, /* 10 */
	  ...
	  { "GOOMBAServerversion", INTFUNC0,GOOMBAServerVersion },
	  { "addslashes", INTFUNC1,_AddSlashes },
	  { "msql_close", INTFUNC0,MsqlClose },
	  { "solid_exec", INTFUNC2,Solid_exec },
	  { "initsyslog", INTFUNC0,GOOMBAServer_init_syslog },
	...
	{ { "msql_result", INTFUNC3,MsqlResult }, /* 11 */
	  ...
	  { "GOOMBAServershowpool",INTFUNC0,ShowPool },
#if HAVE_LIBADABAS
...
int CommandLookup(int cmdlen, YYSTYPE *lvalp) {
	...
	if(cmdlen<=GOOMBAServer_MAX_CMD_LEN) while(cmd_table[cmdlen][i].cmd) {
...
void DefineFunc(char *fnc) {
	...
	len = strlen((char *)fnc);
	if(len<=GOOMBAServer_MAX_CMD_LEN) while(cmd_table[len][i].cmd) {
...
#if APACHE
#if MODULE_MAGIC_NUMBER > 20220110
	rflush(GOOMBAServer_rqst);
#else
	bflush(GOOMBAServer_rqst->connection->client);
#endif
#else
	fflush(stdout);
#endif
