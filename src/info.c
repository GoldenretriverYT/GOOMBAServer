...
void Info(void) {
	...
	env_arr = table_elts(GOOMBAServer_rqst->headers_out);
	env = (table_entry *)env_arr->elts;
	for(i = 0; i < env_arr->nelts; ++i) {
		if(!env[i].key) continue;
		sprintf(buf,"%s:%s<br>\n",env[i].key,env[i].val);
...
#endif
#if APACHE
	path = GOOMBAServer_rqst->filename;
#else	
	...
	if(path) {
		...
		if(stat(path,&sb)!=-1) {
			...
			PUTS("</i></font><br>\n");
#if APACHE
			switch(GOOMBAServer_rqst->finfo.st_mode&S_IFMT) {
#else
...
#if APACHE
				ctime(&GOOMBAServer_rqst->finfo.st_atime));
#else
				ctime(&sb.st_atime));
#endif
			PUTS(buf);
...
#if APACHE
				ctime(&GOOMBAServer_rqst->finfo.st_mtime));
#else
				ctime(&sb.st_mtime));
#endif
			PUTS(buf);
...
#if APACHE
				ctime(&GOOMBAServer_rqst->finfo.st_ctime));
#else
				ctime(&sb.st_ctime));
#endif
			PUTS(buf);
...
void GOOMBAServerVersion(void) {
	Push(GOOMBAServer_VERSION,STRING);
}
