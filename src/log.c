...
#if APACHE
void GOOMBAServer_init_log(GOOMBAServer_module_conf *conf) {
#else
...
void loadlastinfo(char *dbmfile,char *filename) {
	...
	if(!dbmfile) {
		if(lbuf[0]=='\0') {
			if(!filename) {
#if APACHE
				filename = GOOMBAServer_rqst->uri;
#else
...
void GetMyUid(void) {
	...
	if(MyUid==-1) {
		...
		if(path) {
#if APACHE
			MyUid = GOOMBAServer_rqst->finfo.st_uid;
#else
			if(stat(path,&sb)==-1) {
...
#if APACHE
			MyUid = GOOMBAServer_rqst->finfo.st_uid;
			last_modified = GOOMBAServer_rqst->finfo.st_mtime;
#else
			if(stat(path,&sb)==-1) {
				Error("Unable to find: %s",path);
				return(-1L);
...
void GetMyInode(void) {
	...
	if(MyInode==-1) {
		...
		if(path) {
#if APACHE
			MyInode = GOOMBAServer_rqst->finfo.st_ino;
#else
			if(stat(path,&sb)==-1) {
