...
void Exec(char *name, char *retname, int type) {
...
#if GOOMBAServer_SAFE_MODE
	l = strlen(s->strval)+strlen(GOOMBAServer_SAFE_MODE_EXEC_DIR)+2;
	c = strchr(s->strval,' ');
	if(c) *c='\0';
	if(strstr(s->strval,"..")) {
	...
	b = strrchr(s->strval,'/');
	d = emalloc(0,l);
	strncpy(d,GOOMBAServer_SAFE_MODE_EXEC_DIR,l-1);
	if(b) {
		strncat(d,b,l-1);
...
int GOOMBAServer_ind(char *s, char c) {
    register int x;
...
void EscapeShellCmd(void) {
	...
	strcpy(cmd,s->strval);
    for(x=0;cmd[x];x++) {
        if(GOOMBAServer_ind("&;`'\"|*?~<>^()[]{}$\\\x0A\xFF",cmd[x]) != -1){
            for(y=l+1;y>x;y--)
                cmd[y] = cmd[y-1];
