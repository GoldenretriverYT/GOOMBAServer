...
void RegMatch(unsigned char *reg_name) {
	...
	if(s->type==STRING)
		...
		cp = GOOMBAServer_re_compile_pattern(temp3,strlen(temp3),&exp);
	}
	...
	ret = GOOMBAServer_re_match(&exp,string,strlen(string),0,regs);
	if(reg_name) {
		l = strlen(string)+1;
		buf = emalloc(1,l);
		for(i=0;i<10;i++) {
...
void RegSearch(unsigned char *reg_name) {
	...
	GOOMBAServer_re_compile_fastmap(&exp);
	regs = emalloc(1,sizeof(struct re_registers));
	ret = GOOMBAServer_re_search(&exp,string,strlen(string),0,strlen(string),regs);
	if(ret<0) {
		Push("",STRING);
	} else {
...
char *_RegReplace(char *pattern, char *replace, char *string) {
	...
	l = strlen(string);
	if(!l) return(string);
	cp = GOOMBAServer_re_compile_pattern(pattern,strlen(pattern),&exp);
	if(cp) {
		Error("reg_match error: %s",cp);
		return((char *)-1);
	}	
	...
	ll = strlen(replace);
	while(ret>=0) {
		ret = GOOMBAServer_re_search(&exp,string,l,i,l-i,&regs);
		if(ret>=0) {
