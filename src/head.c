...
void Header(void) {
	...
	if(r) {
		*r='\0';
		table_set(GOOMBAServer_rqst->headers_out,s->strval,r+1);
		if(!strcasecmp(s->strval,"location")) GOOMBAServer_rqst->status = REDIRECT;
#if DEBUG
...
void GOOMBAServer_header(int type,char *str) {
    ...
    if((PrintHeader && !HeaderPrinted) || (PrintHeader && HeaderPrinted==2)) { 
        if(HeaderPrinted!=2) GOOMBAServer_rqst->content_type = "text/html";
		if(type==1) {
