...
#if APACHE
void GOOMBAServer_init_acc(GOOMBAServer_module_conf *conf) {
#else
...
char *getremotehostname(void) {
	...
	if(!RemoteHostName) {
#if APACHE
		s = table_get(GOOMBAServer_rqst->subprocess_env,"REMOTE_HOST");
#else
...
#if APACHE
			s = table_get(GOOMBAServer_rqst->subprocess_env,"REMOTE_ADDR");
#else
...
char *getrefdoc(void) {
	if(!RefDoc) {
#if APACHE
		RefDoc = table_get(GOOMBAServer_rqst->subprocess_env,"HTTP_REFERER");
#else
...
char *getbrowser(void) {
	if(!Browser) {
#if APACHE
		Browser = table_get(GOOMBAServer_rqst->subprocess_env,"HTTP_USER_AGENT");
#else
