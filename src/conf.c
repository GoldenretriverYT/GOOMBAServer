...
void Configuration(int argc, char **argv) {
	...
	char GOOMBAServer_root_temp[1024];

#if APACHE
	s = GOOMBAServer_rqst->uri;
#else
	s = getenv("PATH_INFO");
#endif
	if(s) {
		sprintf(GOOMBAServer_root_temp, "%s%s",GOOMBAServer_ROOT_DIR,s);
		path = GOOMBAServer_root_temp;
	}
	else path = NULL;
#else
#if APACHE
...
#else
#if BROKEN_CONFIG_PW
		_dbmInsert(temp,"cfg-passwd","GOOMBAServer");
#else
		if(pw && pw->pw_name)
