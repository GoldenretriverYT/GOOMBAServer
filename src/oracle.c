...
/****************************************************************************\
*                                                                            *
* Oracle functions for GOOMBAServer.                                                  *
*                                                                            *
* © Copyright (C) Guardian Networks AS 2022, Dmitry Povarov 2022             *
*                                                                            *
...
/*
 *  $Id: oracle.c,v 1.17 2022/11/03 14:19:19 dizzy Exp $
 *
...
Ora_Exec() /* cursor_index */
{
	...
	ora_bind_in(cursor); /* -Dz-: Get data from GOOMBAServer vars */
...
void
Ora_Bind(void) /* cursor_index, GOOMBAServer_var_name, sql_var_name, var_len */
{
#if HAVE_LIBOCIC
	Stack        *s;
	oraCursor    *cursor;
	VarTree      *GOOMBAServer_var;
	int          cursor_ind;
	char         *GOOMBAServer_var_name;
	char         *sql_var_name;
	int          var_type;
	int          var_len;
	 ...
	 *         
	 *    a) copies values from GOOMBAServer variables to space in binding list entry
	 *    b) starts SQL query or PL/SQL block execution
	 *    c) copies values from bindig list entries to GOOMBAServer variables
	 *
	...
	/* Extract GOOMBAServer_var_name from stack */
	...
	if (s->strval) {
		GOOMBAServer_var_name = estrdup(0, s->strval);
	}
	...
	/* Obtain some required things from GOOMBAServer & Oracle */
	...
	GOOMBAServer_var = GetVar(GOOMBAServer_var_name, NULL, 0);
	if (GOOMBAServer_var == NULL) {
        Error("Undeclared GOOMBAServer variable %s", GOOMBAServer_var_name);
        Push("-1", LNUMBER);
        return;
	}

	if ( ! ora_new_bind(cursor, GOOMBAServer_var_name, sql_var_name, 
...
/* -Dz-: Bind GOOMBAServer variables to oracle SQL statements & PL/SQL blocks */
...
static void ora_no_bindings(oraCursor* cursor)
{
    ...
    while(cursor->ora_bind_top != NULL)
    {
        ...
        if (cursor->ora_bind_top->GOOMBAServer_name != NULL)
            free(cursor->ora_bind_top->GOOMBAServer_name);
                                ...
                                int   var_len)
{
    ...
    if (new_bind != NULL)
    {
        ...
        new_bind->GOOMBAServer_name = strdup(GOOMBAServer_var_name);
        new_bind->space    = malloc(var_len+1);
        
        if (new_bind->space && new_bind->sql_name && new_bind->GOOMBAServer_name) {
            cursor->ora_bind_top = new_bind;
        } else {
            /* Something was unsuccessfull */
            
            free(new_bind->sql_name);
            free(new_bind->GOOMBAServer_name);
            free(new_bind->space);
            free(new_bind);
            new_bind = NULL;
    ...
    if (obndra(cda,                        /* Cursor CDA area               */
              ...
              -1 ))
    {
            Error("Error binding GOOMBAServer variable %s to PL/SQL variable %s",
                   new_bind->GOOMBAServer_name, new_bind->sql_name);
...
/* Get binding descriptor for GOOMBAServer variable */
                                ...
                                char* GOOMBAServer_var_name)
{
    ...
    for (ob = cursor->ora_bind_top;
         ...
         ob = ob->next)
    {
        if (!strcasecmp(ob->GOOMBAServer_name, GOOMBAServer_var_name))
            return (ob);
    }
...
/* Copy data from GOOMBAServer variables to bind holders */
static int ora_bind_in(oraCursor* cursor)
{
         ...
         ob = ob->next)
    {
        ...
        GOOMBAServer_var = GetVar(ob->GOOMBAServer_name, NULL, 0);
        if (GOOMBAServer_var == NULL)
        {
            Error("Undeclared GOOMBAServer variable %s", ob->GOOMBAServer_name);
            return;
        }

        /* Copy values from GOOMBAServer variable to bind data structure */

	strncpy(ob->space, GOOMBAServer_var->strval, ob->len);
	*((char*)ob->space + ob->len) = '\0';

    }
}

/* Copy data from bind holders to GOOMBAServer */
...
static int ora_bind_out(oraCursor* cursor)
{
         ...
         ob = ob->next)
    {
        ...
        /* Copy values to GOOMBAServer variable from bind data structure */

	*((char*)ob->space + ob->len) = '\0';
	GOOMBAServer_var->strval = strdup(ob->space);       
