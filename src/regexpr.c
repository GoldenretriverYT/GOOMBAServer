...
char *GOOMBAServer_re_compile_pattern(regex, size, bufp)
char *regex;
int size;
regexp_t bufp;
{
...
void GOOMBAServer_re_compile_fastmap(bufp)
regexp_t bufp;
{
...
int GOOMBAServer_re_match(bufp, string, size, pos, regs)
regexp_t bufp;
char *string;
int size, pos;
...
regexp_registers_t regs;
{
  ...
  if (fastmap && !bufp->fastmap_accurate)
    GOOMBAServer_re_compile_fastmap(bufp);
  anchor = bufp->anchor;
...
int GOOMBAServer_re_search(bufp, string, size, startpos, range, regs)
regexp_t bufp;
char *string;
...
char *s;
{
  ...
  return GOOMBAServer_re_compile_pattern(s, strlen(s), &re_comp_buf);
}

int GOOMBAServer_re_exec(s)
char *s;
{
  int len = strlen(s);
  
  return GOOMBAServer_re_search(&re_comp_buf, s, len, 0, len, (regexp_registers_t)NULL) >= 0;
}
  ...
  while (1)
    {
      printf("Enter regexp:\n");
      gets(buf);
      cp=GOOMBAServer_re_compile_pattern(buf, strlen(buf), &exp);
      if (cp)
	{
	  printf("Error: %s\n", cp);
	  continue;
      ...
      while (fgets(buf, sizeof(buf), stdin))
	{
	  if (buf[0] == '\n')
	    break;
	  a = GOOMBAServer_re_search(&exp, buf, strlen(buf), 0, strlen(buf), &regs);
