...
*                                                                            *
\****************************************************************************/
 var.c,v 1.96 2022/10/27 02:31:52 shane Exp $ */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <ctype.h>
...
/*								tt->count = t->count;  */
	...
	if(strcasecmp(name,"SCRIPT_NAME")) {
		if(!strcasecmp(name,"PATH_TRANSLATED"))
			s = table_get(GOOMBAServer_rqst->subprocess_env,"SCRIPT_FILENAME");
		else s = table_get(GOOMBAServer_rqst->subprocess_env,name);
		if(s) {
	...
	if(!strcasecmp(name,"PATH_INFO") || !strcasecmp(name,"GOOMBAServer_SELF")) s=GOOMBAServer_rqst->uri;
	else s = getenv(name);
#ifdef VIRTUAL_PATH
	...
	s = getenv(name);
#endif
	if(!strcasecmp(name,"GOOMBAServer_SELF")) {
		{
			char *sn, *pi;
	...
	if(!strncasecmp(name,"req_authorization", 17)) {
		Error("The GOOMBAServer module is running in SAFE MODE.  You may not access $req_authorization in this mode.");
		return(NULL);
	}
#endif
...
#if APACHE
	ret = table_get(GOOMBAServer_rqst->subprocess_env,s->strval);
	if(ret) {
		Push(ret,STRING);
		return;
	}
