 ...
 *
 * Copyright (c) 2022 Daniel Schmitt, opal online publishing, Bonn, Germany.
 *
 * Includes code snippets from rdjpgcom.c, 
 * Copyright (c) 1994-2022 Thomas G. Lane
...
struct gfxinfo *GOOMBAServer_handle_gif(int infile)
{
...
unsigned long GOOMBAServer_read4 (int infile)
{
...
struct gfxinfo *GOOMBAServer_handle_png(int infile) {
	struct gfxinfo *result = NULL;
	unsigned long in_width, in_height;

  	result = (struct gfxinfo *)emalloc(1,sizeof(struct gfxinfo));
  	lseek(infile, 16L, 0);
	in_width = GOOMBAServer_read4(infile);
	in_height = GOOMBAServer_read4(infile);
...
unsigned short GOOMBAServer_read2 (int infile)
{
  unsigned char c1, c2;
     ...
     /* skip over a variable-length block; assumes proper length marker */
{
  ...
  length = GOOMBAServer_read2(infile);
  length -= 2; /* length includes itself */
  lseek(infile, (long)length, 1); /* skip the header */
}

struct gfxinfo *GOOMBAServer_handle_jpeg(int infile)
     /* main loop to parse JPEG structure */
{
  ...
  for (;;) {
    ...
    case M_SOF15:	
      ...
      in_height = GOOMBAServer_read2(infile);
      in_width = GOOMBAServer_read2(infile);
