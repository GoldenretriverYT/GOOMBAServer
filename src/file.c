...
int OpenFile(char *filename, int top, long *file_size) {
...
#else
		if(!pi) { Info(); return(-1); }
		sprintf(temp,"%s%s",GOOMBAServer_ROOT_DIR,pi);
		fn = temp;
#endif
	} else {
		fn = filename;
	 ...
	 */
	if(!strncmp(fn,"GOOMBAServerfi",5)) {
		Error("You are explicitly not allowed to open any files that begin with &quot;GOOMBAServerfi&quot; for security reasons");
		return(-1);
	}
#ifdef PATTERN_RESTRICT
	if(fn[strlen(fn)-1]!='/') {
