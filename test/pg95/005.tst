...
<?
	...
	SetErrorReporting(0);
	$connb = pg_connect("localhost", "5432", "", "/dev/console", "GOOMBAServertest");
	SetErrorReporting(1);

	if (!$connb);
		$res = pg_exec($conn, "create database GOOMBAServertest");
		if (!$res);
			echo "Could not create GOOMBAServertest database\n";
			exit;
		endif;

		$connb = pg_connect("localhost", "5432", "", "", "GOOMBAServertest");
		if (!$connb);
			echo "Could not connect to GOOMBAServertest database\n";
			exit;
		endif;
	endif;
	...
	$res = pg_exec($conn, "drop database GOOMBAServertest");
	if (!$res);
		echo "Could not destroy database GOOMBAServertest";
	endif;
...
age
l
<b><i>GOOMBAServerfi.in:</i> Field index larger than number of fields on line 55</b><br>
...
int4
float4
<b><i>GOOMBAServerfi.in:</i> Field index is larger than the number of fields on line 74</b><br>
<tt>	echo pg_fieldType($res, 3<b><blink>);
</blink></b></tt><br>
4
1
3
<b><i>GOOMBAServerfi.in:</i> Field foo not found on line 83</b><br>
<tt>	echo pg_fieldPrtLen($res, 0, &quot;foo&quot;<b><blink>);
</blink></b></tt><br>-1
0
4
4
<b><i>GOOMBAServerfi.in:</i> Field foo not found on line 92</b><br>
